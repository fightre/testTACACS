...
public class Header
{
  ...
  public static byte TYPE_AUTHENTIC = 1;
  public static byte TYPE_AUTHORIZE = 2;
  public static byte TYPE_ACCOUNTIN = 3;
  ...
  static byte extractVersionNumber(byte[] Header) { return Header[0]; }
  
  static byte extractFlags(byte[] Header) {
    return Header[3];
  }
  
  static byte extractSeqNum(byte[] Header) { return Header[2]; }
  ...
  static int extractBodyLen(byte[] header) { byte[] length = new byte[4];
    length[0] = header[8];
    length[1] = header[9];
    length[2] = header[10];
    length[3] = header[11];
  ...
  static byte[] generateSessionID() { Random ran = new Random(System.currentTimeMillis());
